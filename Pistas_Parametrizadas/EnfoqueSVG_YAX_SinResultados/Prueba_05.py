...
def scale_track_segments(segs, desired_length_m=400.0):
    ...
    for seg in segs:
        if seg['type'] == 'straight':
        ...
        else:  # curva: muestreamos 50 puntos y sumamos distancias
            ctr = np.array(seg['controls'])
            ...
            total_len += np.sum(np.linalg.norm(np.diff(curve, axis=0), axis=1))

    # 2) factor de escala
    scale = desired_length_m / total_len
    print(f"Longitud actual = {total_len:.2f} m, scale = {scale:.4f}")

    # 3) aplicar escala a todos los puntos
    for seg in segs:
        if seg['type'] == 'straight':
            seg['p0'] = (np.array(seg['p0']) * scale).tolist()
            seg['p1'] = (np.array(seg['p1']) * scale).tolist()
        else:
            seg['controls'] = (np.array(seg['controls']) * scale).tolist()
    return segs
...
def load_and_plot(svg_path):
    ...
    segs = scale_track_segments(segs, desired_length_m=400.0)

    # Preparamos el CSV
    out_dir = 'param_csv'
    os.makedirs(out_dir, exist_ok=True)
    name = os.path.splitext(os.path.basename(svg_path))[0]
    csv_path = os.path.join(out_dir, f"{name}_points.csv")
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        # Diccionario para leyendas en el plot (opcional)
        seen = {
        ...
        for seg in segs:
            if seg['type'] == 'straight':
                p0 = np.array(seg['p0'])
                p1 = np.array(seg['p1'])
                # 1) Saltar rectas degeneradas
                if np.allclose(p0, p1, atol=1e-6):
                    continue

                # 2) Sólo dos puntos: inicio y fin
                pts = np.vstack([p0, p1])
                inn, out = offset_curves(pts)

                for c, i, o in zip(pts, inn, out):
                    writer.writerow([
                        c[0], c[1],
                        o[0], o[1],    # border1 = externo
                        i[0], i[1],    # border2 = interno
                        0
                    ])
        ...
        plt.title("Pista con cotas y Puntos curva")
        plt.xlabel("x")
        plt.ylabel("y")
        plt.legend()
        plt.grid(True)
