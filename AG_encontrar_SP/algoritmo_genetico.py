...
from funciones_AG_pista import inicializar_poblacion

"Operaciones del Algoritmo Genético"

# Parámetros
ancho_extra = 15 # Ancho de la línea extra
    # Escalar el circuito
desired_length = 4304  # Longitud deseada
tam_pob = 200 # tamaño de población
...
# Ejemplo de uso de la función de inicialización (puedes ajustar los parámetros)
# Solo se aplica al primer path del archivo

poblacion = inicializar_poblacion(paths[0], num_pob=tam_pob, num_var=1, largo_real=desired_length, ancho_real=ancho_extra)

# Graficar el circuito escalado con la segunda línea y la población inicial
plt.figure(figsize=(10, 8))
plt.plot(x_scaled, y_scaled, 'k-', linewidth=2, label="Línea interna")
plt.plot(x_offset, y_offset, 'r-', linewidth=2, label="Línea externa")

# Dibujar población inicial
if poblacion:
    poblacion_x, poblacion_y = zip(*poblacion)
    plt.scatter(poblacion_x, poblacion_y, c='blue', s=10, label="Población inicial")
