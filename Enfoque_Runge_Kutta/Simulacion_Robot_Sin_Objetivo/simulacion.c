...
// Prototipos de funciones auxiliares (para evitar ambigüedades, se renombran las funciones helper)
...
//
// La función genera en cada iteración dos números aleatorios para u1 y u2 (aceleración de cada llanta),
// ejecuta Runge–Kutta para actualizar el estado y guarda los datos en un arreglo que se retorna.
EXPORT SimulationResult* simular(float delta_t, float limite_tiempo,
    ...
    float x = cond_x;
    float y = cond_y;
    float phi_val = cond_phi; // Se utiliza "phi_val" para diferenciar de la función helper
    float vl = cond_vl;
    float vr = cond_vr;
    float u1, u2;
    ...
    for (int i = 0; i < n_iter; i++) {
        // Generamos u1 y u2 aleatorios en el intervalo [-0.5, 0.5]
        u1 = ((float)rand() / RAND_MAX) - 0.5f;
        u2 = ((float)rand() / RAND_MAX) - 0.5f;
...
float f_x(float vl, float vr, float phi) {
    return ((vl + vr) / 2.0f) * cos(phi);
}

float f_y(float vl, float vr, float phi) {
    return ((vl + vr) / 2.0f) * sin(phi);
}
