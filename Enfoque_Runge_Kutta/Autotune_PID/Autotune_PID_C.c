...
void crear_poblacion(poblacion *poblacion, int longitud_genotipo, double delta_t, double B, double limite_inferior, double limite_superior, PuntoObjetivo *objetivos, int num_objetivos, double distancia_umbral)
{
    for (int i = 0; i < poblacion->tamano; i++)
    {
        ...
        double kP = ((double)rand() / RAND_MAX) * 0.0001;
        double kI = ((double)rand() / RAND_MAX) * 0.0000001;
        double kD = ((double)rand() / RAND_MAX) * 0.00000000001;
        ...
        for (int j = 1; j < longitud_genotipo; j++)
        {
            ...
            // Imprimir los valores de x, y, phi_val, vl, vr, u1 y u2
            printf("x: %f, y: %f, phi_val: %f, vl: %f, vr: %f, u1: %f, u2: %f\n", x, y, phi_val, vl, vr, u1, u2);
...
int main(int argc, char **argv)
{
    ...
    srand(time(NULL));
    int tamano_poblacion = 1; // Tamaño de la población
    double delta_t = 0.01; // Paso de tiempo
    double tiempo_test = 20.0; // Tiempo total de simulación
   ...
   // Definir puntos objetivo (ejemplo)
    ...
    // Escribir encabezado (añadir u1 y u2)
    fprintf(fp, "time,x,y,u1,u2\n");  // Nuevo encabezado
    ...
    for (int j = 0; j < longitud_genotipo; j++)
    {
        // Leer u1 y u2 del genotipo (valores originales)
        ...
        // Guardar datos incluyendo u1 y u2 del genotipo original
        fprintf(fp, "%.4f,%.4f,%.4f,%.4f,%.4f\n",  // Nuevos campos
                t,
                x_pos,
                y_pos,
                Mejor_Individuo->genotipo_izquierdo[j],  // u1 original
                Mejor_Individuo->genotipo_derecho[j]);    // u2 original
    }
