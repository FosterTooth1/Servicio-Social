...
void determinar_limites_interno_externo(const Punto *p, double *xi, double *yi, double *xe, double *ye) {
    double dx1 = p->x_border1 - p->x_central;
    double dy1 = p->y_border1 - p->y_central;
    double d1_2 = dx1*dx1 + dy1*dy1;

    double dx2 = p->x_border2 - p->x_central;
    double dy2 = p->y_border2 - p->y_central;
    double d2_2 = dx2*dx2 + dy2*dy2;

    if (d1_2 <= d2_2) {
        // border1 es interno, border2 es externo
        *xi = p->x_border1;
        *yi = p->y_border1;
        *xe = p->x_border2;
        *ye = p->y_border2;
    } else {
        // border2 es interno, border1 es externo
        *xi = p->x_border2;
        *yi = p->y_border2;
        *xe = p->x_border1;
        *ye = p->y_border1;
...
void guardar_solucion_csv(const char *nombre_archivo, Solucion *sol, int num_puntos) {
    ...
    fprintf(f, "x_sol,y_sol,x_border1,y_border1,x_border2,y_border2\n");
    for (int i = 0; i < num_puntos; ++i) {
